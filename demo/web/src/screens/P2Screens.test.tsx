  ...
  const fetchMock = vi.fn((request: RequestInfo | URL) => {
    ...
    if (url.pathname === "/api/memory/bands") {
      return Promise.resolve(
        jsonResponse({
          bands: [
            { id: "episodic", n: "01", name: "episodic", desc: "what happened", count: 0, growth: [1], stats: [] },
            { id: "semantic", n: "02", name: "semantic", desc: "beliefs", count: 0, growth: [1], stats: [] },
            { id: "procedural", n: "03", name: "procedural", desc: "skills", count: 1, growth: [1], stats: [{ k: "skills", v: 1 }] },
            { id: "affective", n: "04", name: "affective", desc: "mood", count: 0, growth: [1], stats: [] },
            { id: "self", n: "05", name: "self", desc: "identity", count: 0, growth: [1], stats: [] },
            { id: "commitments", n: "06", name: "commitments", desc: "rules", count: 0, growth: [1], stats: [] },
            { id: "social", n: "07", name: "social", desc: "people", count: 0, growth: [1], stats: [] },
            { id: "relational", n: "08", name: "relational", desc: "slots", count: 0, growth: [1], stats: [] }
          ]
        })
      );
    }
    ...
    if (url.pathname === "/api/state") {
      return Promise.resolve(
        jsonResponse({
          ...
          current_mood: { session_id: "default", valence: 0, arousal: 0, updated_at: 1, half_life_hours: 1, recent_triggers: [] },
          version: "test"
        })
      );
    }
          ...
          entries: [
            {
              ...
              superseded_by_id: null,
              rank: 0,
              canonicalizes: { goal_ids: [], commitment_ids: ["cm_1"], action_ids: [], open_question_ids: [] }
            }
          ]
        })
      );
    ...
    if (url.pathname === "/api/dream/state") {
      return Promise.resolve(
        jsonResponse({
          ...
          schedule: [{ process: "belief-reviser", scheduled_at: 4, source: "stream", stream_entry_id: "strm_dream" }],
          audit_rows: [],
          belief_revision_rows: [
            {
              id: 1,
              kind: "belief_revision",
              refs: { target_type: "semantic_node", target_id: "sn_1", invalidated_edge_id: "se_1" },
  ...
  it("renders stream live tail and attachment quarantine detail", async () => {
    ...
    act(() => {
      live.emit({
        type: "stream:append",
        ts: 5,
        entries: [streamEntry({ id: "strm_live", timestamp: 5, kind: "dream_report", content: { processes: ["curator"] } })]
      });
    });
