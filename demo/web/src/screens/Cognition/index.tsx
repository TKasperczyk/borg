...
import { getStream, getTurns, setSessionPolicy } from "../../api/client";
import type {
...
import type { TurnFlowSnapshot, TurnStreamState } from "../../hooks/use-turn-stream";
import {
  STREAM_OUTCOME_DESCRIPTORS,
  type StreamOutcomeClass,
} from "../../lib/stream-outcomes";
import { formatTime, mergeEntries, sortStreamEntries, streamContentText } from "../../lib/stream-utils";
...
const TRACE_NOT_RETAINED_MESSAGE =
...
function isTurnHistoryRelevantAppendEntry(entry: StreamEntry): boolean {
  if (
  ...
  return (
    ...
    (entry.turn_status === "aborted" ||
      (isRecord(entry.content) && entry.content.event === "aborted_turn"))
  );
}
...
export function CognitionScreen({
  ...
  const [historyFilter, setHistoryFilter] = useState<TurnOutcomeFilter>("all");
  ...
  useEffect(() => {
    return live.subscribe((frame) => {
      ...
      if (
        frame.entries.some(
          (entry) => entry.session_id === sessionId && isTurnHistoryRelevantAppendEntry(entry),
        )
      ) {
        void refetchTurns();
      }
    });
  }, [audience, live, refetchTurns, sessionId]);
  ...
  const historyRows = turnsApi.data?.rows ?? [];
  const selectedSnapshot =
    selectedTurnId === null ? undefined : turnStream.flowSnapshotByTurn.get(selectedTurnId);
  const selectedLedger =
    selectedTurnId === null ? undefined : turnStream.ledgerByTurn.get(selectedTurnId);
  const liveLedger =
    turnStream.activeTurnId === null ? undefined : turnStream.ledgerByTurn.get(turnStream.activeTurnId);
  const historicalView = selectedTurnId !== null;
  const retainedSnapshot: TurnFlowSnapshot | null =
    historicalView && selectedSnapshot !== undefined ? selectedSnapshot : null;
  const tracePlaceholder =
    historicalView && retainedSnapshot === null ? TRACE_NOT_RETAINED_MESSAGE : null;
  const xrayState = retainedSnapshot ?? {
  ...
  const xrayTurnId =
    historicalView && retainedSnapshot !== null ? selectedTurnId : turnStream.activeTurnId;
  const xrayLedger = historicalView ? selectedLedger : liveLedger;
