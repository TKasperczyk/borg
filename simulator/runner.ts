...
function primaryNoOutputReasonFromCategories(categories: readonly string[]): string {
...
function structuralNoOutputFlagsFromCategories(categories: readonly string[]): string[] {
  const flags: string[] = [];
...
function incrementStringCounts(counts: Record<string, number>, labels: readonly string[]): void {
  for (const label of labels) {
    counts[label] = (counts[label] ?? 0) + 1;
  }
}

function incrementNestedStringCounts(
...
function postGenerationRejectedForTurn(
  ...
  const noOutputCategories = Array.isArray(record?.no_output_categories)
    ...
    : [];
  const explicitPrimaryNoOutputReason =
    typeof record?.primary_no_output_reason === "string" ? record.primary_no_output_reason : null;
  const explicitStructuralNoOutputFlags = Array.isArray(record?.structural_no_output_flags)
    ? record.structural_no_output_flags.filter((flag): flag is string => typeof flag === "string")
    : [];

  return {
    ...(typeof reason === "string" ? { reason: reason as GenerationSuppressionReason } : {}),
    noOutputCategories,
    primaryNoOutputReason:
      explicitPrimaryNoOutputReason ?? primaryNoOutputReasonFromCategories(noOutputCategories),
    structuralNoOutputFlags:
      explicitStructuralNoOutputFlags.length > 0
        ? [...new Set(explicitStructuralNoOutputFlags)]
        : structuralNoOutputFlagsFromCategories(noOutputCategories),
  };
}
...
export class SimulatorRunner {
  ...
  async run(): Promise<SimulatorRunReport> {
    ...
    try {
      ...
      const finalizerNoOutputPrimaryByReason: Record<string, number> = {};
      const finalizerNoOutputFlagsByFlag: Record<string, number> = {};
      const finalizerNoOutputFlagsByPrimaryReason: Record<string, Record<string, number>> = {};
      let finalizerNoOutputWhenBorgAddressedWithStateDeltaTotal = 0;
      let finalizerNoOutputClosureWithOpenQuestionTotal = 0;
      const finalizerNoOutputMetricContext = () => ({
        finalizerNoOutputByCategory,
        finalizerNoOutputPrimaryByReason,
        finalizerNoOutputFlagsByFlag,
        finalizerNoOutputFlagsByPrimaryReason,
        finalizerNoOutputWhenBorgAddressedWithStateDeltaTotal,
        finalizerNoOutputClosureWithOpenQuestionTotal,
      });
      ...
      for (let turn = 1; turn <= this.options.totalTurns; turn += 1) {
        ...
        try {
          ...
          await metrics.captureAborted(transport.getBorg(), turn, {
            ...
            ...finalizerNoOutputMetricContext(),
          });
        ...
        if (!success.emitted && !isObserveTurn && suppressionReason !== undefined) {
          ...
          if (suppressionReason === "finalizer_no_output") {
            ...
            incrementStringCounts(finalizerNoOutputPrimaryByReason, [
              postGenerationRejection.primaryNoOutputReason,
            ]);
            incrementStringCounts(
              finalizerNoOutputFlagsByFlag,
              postGenerationRejection.structuralNoOutputFlags,
            );
            incrementNestedStringCounts(
              finalizerNoOutputFlagsByPrimaryReason,
              postGenerationRejection.primaryNoOutputReason,
              postGenerationRejection.structuralNoOutputFlags,
            );
            if (
              postGenerationRejection.primaryNoOutputReason === "when_borg_addressed" &&
              postGenerationRejection.structuralNoOutputFlags.includes("with_state_delta")
            ) {
              finalizerNoOutputWhenBorgAddressedWithStateDeltaTotal += 1;
            }
            if (
              postGenerationRejection.primaryNoOutputReason === "closure" &&
              postGenerationRejection.structuralNoOutputFlags.includes("with_open_question")
            ) {
              finalizerNoOutputClosureWithOpenQuestionTotal += 1;
            }
          }
...
function reportNestedCountMap(counts: Record<string, Record<string, number>>): string {
  const entries = Object.entries(counts)
    .map(([label, nested]) => [label, reportCountMap(nested)] as const)
    .filter(([, nested]) => nested !== "none")
  ...
  return entries.map(([label, nested]) => `${label}=[${nested}]`).join("; ");
}

function reportPercent(value: number): string {
...
export function formatSimulatorReport(report: SimulatorRunReport): string {
  ...
  const finalizerNoOutputPrimaryByReason = reportCountMap(
    report.finalMetrics.finalizer_no_output_primary_by_reason,
  );
  const finalizerNoOutputFlagsByFlag = reportCountMap(
    report.finalMetrics.finalizer_no_output_flags_by_flag,
  );
  const finalizerNoOutputFlagsByPrimaryReason = reportNestedCountMap(
    report.finalMetrics.finalizer_no_output_flags_by_primary_reason,
  );
  const finalizerNoOutputTotal = Object.values(
    report.finalMetrics.finalizer_no_output_primary_by_reason,
  ...
  const lines = [
    ...
    `- Shared-state cap pressure: at cap turns ${report.finalMetrics.shared_state_at_cap_turns}/${report.finalMetrics.shared_state_compile_evaluated_turns} evaluated compiles, omitted recent entries ${report.finalMetrics.shared_state_omitted_recent_entries_total_across_compiles}, live starvation ever ${report.finalMetrics.shared_state_live_starvation_ever}, live starvation final ${report.finalMetrics.shared_state_live_starvation_final}, newest reserved ${report.finalMetrics.shared_state_newest_entries_reserved}`,
    "- Shared-state omission (final compile / cumulative):",
    ...
    `  - dormant live final: ${report.finalMetrics.shared_state_omitted_dormant_live_final_compile}`,
    "- Shared-state locked omission severity (final compile / cumulative):",
    `  - recent: ${report.finalMetrics.shared_state_omitted_locked_recent_final_compile} / ${report.finalMetrics.shared_state_omitted_locked_recent_total_across_compiles}`,
    `  - old: ${report.finalMetrics.shared_state_omitted_locked_old_final_compile} / ${report.finalMetrics.shared_state_omitted_locked_old_total_across_compiles}`,
    `  - unknown age: ${report.finalMetrics.shared_state_omitted_locked_unknown_age_final_compile} / ${report.finalMetrics.shared_state_omitted_locked_unknown_age_total_across_compiles}`,
    `  - with active critical commitment: ${report.finalMetrics.shared_state_omitted_locked_with_active_critical_commitment_final_compile} / ${report.finalMetrics.shared_state_omitted_locked_with_active_critical_commitment_total_across_compiles}`,
    `  - with operational canonicalizer: ${report.finalMetrics.shared_state_omitted_locked_with_operational_canonicalizer_final_compile} / ${report.finalMetrics.shared_state_omitted_locked_with_operational_canonicalizer_total_across_compiles}`,
    `  - indexed only: ${report.finalMetrics.shared_state_omitted_locked_indexed_only_final_compile} / ${report.finalMetrics.shared_state_omitted_locked_indexed_only_total_across_compiles}`,
    "- Shared-state at-cap severity:",
    `  - at cap with all keys indexed: ${report.finalMetrics.shared_state_at_cap_but_all_keys_indexed_compiles_total}`,
    `  - at cap with operational omission: ${report.finalMetrics.shared_state_at_cap_with_operational_omission_compiles_total}`,
    `  - at cap with cap rejection: ${report.finalMetrics.shared_state_at_cap_with_cap_rejection_compiles_total}`,
    `- Shared-state lifecycle: active low-salience ${report.finalMetrics.shared_state_active_low_salience_live_final_compile}, active dormant ${report.finalMetrics.shared_state_active_dormant_live_final_compile}, demoted live->low ${report.finalMetrics.shared_state_demoted_live_to_low_salience_total}, demoted low->dormant ${report.finalMetrics.shared_state_demoted_low_salience_to_dormant_total}, reactivated low ${report.finalMetrics.shared_state_reactivated_low_salience_live_total}, reactivated dormant ${report.finalMetrics.shared_state_reactivated_dormant_live_total}`,
    "- Lifecycle aging (live -> low_salience):",
    `  - demotable: ${report.finalMetrics.shared_state_lifecycle_aging_demotable_final_compile} (final compile)`,
    `  - unknown age: ${report.finalMetrics.shared_state_lifecycle_aging_unknown_age_final_compile} (final compile)`,
    `  - demoted: ${report.finalMetrics.shared_state_lifecycle_aging_demoted_total} (cumulative)`,
    `  - blocked by: current_turn=${report.finalMetrics.shared_state_lifecycle_aging_blocked_by_current_turn_update_total}, patch_touch=${report.finalMetrics.shared_state_lifecycle_aging_blocked_by_patch_touch_total}, ledger_overlap=${report.finalMetrics.shared_state_lifecycle_aging_blocked_by_ledger_overlap_total}, recent_retrieval=${report.finalMetrics.shared_state_lifecycle_aging_blocked_by_recent_retrieval_total}, active_canonicalizer_critical=${report.finalMetrics.shared_state_lifecycle_aging_blocked_by_active_canonicalizer_critical_total}, active_canonicalizer_operational=${report.finalMetrics.shared_state_lifecycle_aging_blocked_by_active_canonicalizer_operational_total}, hard=${report.finalMetrics.shared_state_lifecycle_aging_blocked_by_hard_total}, soft=${report.finalMetrics.shared_state_lifecycle_aging_blocked_by_soft_total}, multiple=${report.finalMetrics.shared_state_lifecycle_aging_blocked_by_multiple_reasons_total}`,
    "- Lifecycle aging (low_salience -> dormant):",
    `  - demotable: ${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_demotable_final_compile} (final compile)`,
    `  - unknown age: ${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_unknown_age_final_compile} (final compile)`,
    `  - demoted: ${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_demoted_total} (cumulative)`,
    `  - blocked by: current_turn=${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_blocked_by_current_turn_update_total}, patch_touch=${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_blocked_by_patch_touch_total}, ledger_overlap=${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_blocked_by_ledger_overlap_total}, recent_retrieval=${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_blocked_by_recent_retrieval_total}, active_canonicalizer_critical=${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_blocked_by_active_canonicalizer_critical_total}, active_canonicalizer_operational=${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_blocked_by_active_canonicalizer_operational_total}, hard=${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_blocked_by_hard_total}, soft=${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_blocked_by_soft_total}, multiple=${report.finalMetrics.shared_state_lifecycle_aging_low_salience_to_dormant_blocked_by_multiple_reasons_total}`,
    ...
    `- Finalizer no-output (${finalizerNoOutputTotal} total):`,
    `  - Primary by reason: ${finalizerNoOutputPrimaryByReason}`,
    `  - Structural flags by flag: ${finalizerNoOutputFlagsByFlag}`,
    `  - Flags by primary reason: ${finalizerNoOutputFlagsByPrimaryReason}`,
    "  - Triage combinations:",
    `    - when_borg_addressed + with_state_delta: ${report.finalMetrics.finalizer_no_output_when_borg_addressed_with_state_delta_total}`,
    `    - closure + with_open_question: ${report.finalMetrics.finalizer_no_output_closure_with_open_question_total}`,
    `  - Compatibility categories: ${finalizerNoOutputCategories}`,
    ...
    `- Semantic gate rejections: ungrounded relationship claims ${report.finalMetrics.semantic_nodes_rejected_ungrounded_claim_total} total (${reportCountMap(report.finalMetrics.semantic_nodes_rejected_ungrounded_claim_by_label_family)})`,
    `- Commitment gate rejections: ungrounded relationship claims ${report.finalMetrics.commitment_candidates_rejected_ungrounded_claim_total} total (${reportCountMap(report.finalMetrics.commitment_candidates_rejected_ungrounded_claim_by_label_family)})`,
    "",
    ...
    "- Shared-state empty-update:",
    `  - update operations checked: ${report.finalMetrics.shared_state_update_checked_for_empty_total}`,
    `  - empty updates dropped: ${report.finalMetrics.shared_state_empty_update_dropped_total}`,
    `  - drop rate: ${reportPercent(report.finalMetrics.shared_state_empty_update_drop_rate)}`,
    `  - repaired: ${report.finalMetrics.shared_state_empty_update_repaired_total}`,
    ...
    `- Shared-state gate rejections: ungrounded relationship claims ${report.finalMetrics.shared_state_operations_rejected_ungrounded_claim_total} total (${reportCountMap(report.finalMetrics.shared_state_operations_rejected_ungrounded_claim_by_label_family)})`,
    "",
