...
// Spawn node with the tsx loader directly: a package-manager exec treats the fixture cwd as a
// foreign project and tries to reinstall or purge node_modules (the repo is npm-based).
...
async function runHeuristicsGuardScript(cwd: string): Promise<{
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [...tsxArgs, heuristicsGuardScriptPath], {
      cwd,
      env: {
        ...process.env,
        FORCE_COLOR: "0",
  ...
  maybeIt(
    ...
    async () => {
      const result = await new Promise<{
        ...
        const child = spawn(process.execPath, [...tsxArgs, "scripts/debug.ts"], {
          cwd: repoRoot,
          env: {
            ...process.env,
            FORCE_COLOR: "0",
...
describe("heuristics guard", () => {
  ...
  maybeIt(
    "fails broad frame-anomaly degraded fallback patterns",
    async () => {
      const fixturePath = join(
        guardRoot,
        "src/cognition/frame-anomaly/heuristics-guard-fixture.ts",
      );
      writeFileSync(
        fixturePath,
  ...
  maybeIt(
    "fails disclosure search calls from cognition paths",
    async () => {
  ...
  maybeIt(
    "fails disclosure-suffixed search calls from cognition paths",
    async () => {
  ...
  maybeIt(
    "fails aliased disclosure search calls from cognition paths",
    async () => {
  ...
  maybeIt(
    "fails model-facing memory rows without disclosure labels",
    async () => {
  ...
  maybeIt(
    "fails nested private rows even when an ancestor has disclosure labels",
    async () => {
  ...
  maybeIt(
    "fails unlabeled private rows returned by serializer helper functions",
    async () => {
  ...
  maybeIt(
    "fails non-labeling raw record-copy passthroughs in model-facing payloads",
    async () => {
  ...
  maybeIt(
    "allows labeled serializers, inline labeled objects, fallback record copies, and live-turn text",
    async () => {
  ...
  maybeIt(
    "fails unlabeled shared-state text rows in prompt serializers",
    async () => {
