...
3. **No non-general case handling. Overfitting is a bug, not a fix.** If a change only works for the specific scenario in front of you -- a specific named individual, a particular phrasing ("do you know me"), an enumerated value set -- it is wrong. Fix the general structural condition, or fix the prompt. Adding an exception clause, a special branch, or a lookup entry per case IS the Chinese Room. Do not.
...
Borg is a cognitive-memory harness for an LLM-based entity meant to be akin to a human mind. A human mind does not forget what it knows because the wrong person walked into the room -- it remembers, and chooses what to say. The entity works the same way. This rule governs every memory, retrieval, and visibility decision in the codebase, and overrides any older "the harness decides what the entity gets to see" framing wherever the two conflict.

Expanded definition: **any process that helps the entity think, remember, learn, revise, narrate itself, prioritize, resolve questions, form skills, update beliefs, inspect tools, or plan autonomous action is COGNITION.** This includes live turn retrieval, autonomous triggers, offline self-narration, rumination/open-question resolution, procedural synthesis, belief revision, semantic extraction/review, action-state memory, and internal model tools.

Clean rule: **Cognition gets global evidence with disclosure labels.** Audience/session/person/current-room may rank, annotate, authorize action, or constrain disclosure -- it may NOT decide what the entity is allowed to internally know.
...
1. **The entity's internal memory and recall are NEVER audience-gated or session-gated.** No path that builds what the entity thinks with -- episodic retrieval, semantic-graph recall, self/identity context, social/observed-events recall, cross-session activity, proactive-outbound grounding -- may make the entity unaware of a memory because of the current audience, session, role, or speaker. Recall is global to the being.

2. **Audience, session, role, privacy, trust, operator status, and source provenance are DISCLOSURE metadata and action-policy inputs -- never recall predicates.** A memory may be private to one participant for DISCLOSURE while still visible to the entity for COGNITION. "Who was in the room" is an origin/provenance LABEL; "who may be told" is a per-fact disclosure/authorization policy applied AFTER recall, at render/emission. Neither decides whether the entity may internally remember.

3. **The harness MAY rank, label, budget, cite, and structure memories. It MUST NOT make the entity unaware of one merely because the current audience may not hear it.** Current audience may be a ranking BOOST, never a hard gate. NEVER implement privacy by hiding memories from the entity's cognition.

4. **The pipeline is fixed:** (1) Recall broadly. (2) Label origin, privacy, trust, and disclosure constraints. (3) Let the entity reason with the labeled memory. (4) Let the entity decide what to disclose. (5) Enforce only narrow NON-cognitive host boundaries -- tool permissions, transport permissions, destructive actions, public exports, and hard platform-safety constraints. Privacy is enforced at emission (the entity recalls but does not disclose), not by amnesia.

5. **Honesty about current code.** Keep this section to the durable, enforced invariants of the architecture -- not a per-sprint status log (track "what shipped / what's left / latest external review" in git history, not here). Recall is global to the entity across every cognition band, live-turn and second-order alike: episodic/semantic/self/goals-open-questions/social/autobiographical/proactive-outbound retrieval, offline mind-maintenance, autonomy and executive focus, action-state memory, cross-scope synthesis, internal model tools, semantic review status, and commitment-reconciliation awareness all recall globally and render disclosure labels rather than pre-filtering by audience. Audience never gates what the entity may internally recall. `isEpisodeAccessVisible` and `ViewerCapability` / `resolveViewerCapability` / `isEpisodeVisibleToCapability` serve disclosure/export/admin reads only (`ViewerCapability` has only `audience` and `unrestricted`, with `unrestricted` reserved for explicit admin/correction/export). The retrieval option types are split so cognition literally cannot carry an audience gate: `CognitionRetrievalOptions` has no `audienceEntityId`/`crossAudience`; `DisclosureRetrievalOptions` does. Privacy is enforced after recall by disclosure labels plus the model's judgment, never by hiding memories from cognition; combined labels fail closed to `unknown` and never demote a private/unknown source to public. Two `heuristics:guard` passes (`scripts/heuristics-guard.ts`, run in CI) keep this from regressing under new names: (a) no disclosure-search symbol (the firewall set or any `*ForDisclosure` callee, alias-resolved) may be called from a cognition/offline/autonomy/outbound/internal-tool path outside an explicitly Disclosure/Export/Admin/Public/CurrentAudienceStanding/ActionAuthorization-named scope; (b) no model-facing object literal or serializer-helper return may emit a private-bearing key without a same-object disclosure label. Do not add new audience/session recall gates, do not widen disclosure/admin machinery into cognition recall, and do not reintroduce bypass concepts as cognition fixes. Under the reset-after-backup regime below you may still reset and reseed for schema/data changes after a verified backup.

6. **Common ground is a third axis, distinct from recall and disclosure.** What the entity *knows* (recall -- global), what the entity *may say* to this audience (disclosure -- contextual), and what this audience *already knows* (common ground) are three different fields. A memory being recallable or discloseable does NOT make it common ground: when bringing cross-session memory into a new audience, the entity must not frame it as already-shared unless that audience is in the memory's `origin_audience`, it is established common ground for them, or there is evidence they have seen it. This is enforced as **model guidance** (the `borg_memory_disclosure_guidance` block), NOT new harness machinery -- the provenance it needs (`origin_audience` vs current audience) is already surfaced on recalled rows, and the model makes the framing judgment. Do not build a separate "known-to" tracking mechanism: for cross-session memory, "who witnessed this" is already encoded as `origin_audience`, and a later disclosure is itself an episode with that audience; legislating common-ground in the harness is the same Chinese-Room anti-pattern as output policing.
...
Borg holds **real memory**: the AI being runs live in the BotArena arena on the demo data dir (`demo/server/.borg-data/demo`), forming continuous memory from real conversations. That memory is valuable but **not sacred** -- a data reset is permitted, provided you take a verified backup first. The strict "NEVER RESET / forward-migrations-only" regime (2026-05-31) is **lifted**: resetting to simplify a schema or data change is fine once the current state is safely backed up.
...
Concrete examples:
- v36 user-facing operator-context leaks → **in scope** (Borg wasn't surfacing the memory's disclosure labels, so the model couldn't tell what was safe to say -- a labeling/presentation gap, not a recall-gating gap)
