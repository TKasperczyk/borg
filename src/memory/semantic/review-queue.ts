...
const storedReviewKindSchema = z.enum(STORED_REVIEW_KINDS);
...
export type StoredReviewKind = z.infer<typeof storedReviewKindSchema>;
...
export class ReviewQueueHandlerRegistry {
  ...
  get(kind: StoredReviewKind): ReviewQueueHandler<ReviewKind, unknown, unknown> | null {
    const parsed = reviewKindSchema.safeParse(kind);

    if (!parsed.success) {
      return null;
    }

    return this.handlers.get(parsed.data) ?? null;
  }
}
