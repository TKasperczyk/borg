...
export function isTerminalActionState(state: ActionRecord["state"]): boolean {
  return isActionTerminalClosureGuardState(state);
}

export type CompleteActionRepository = Pick<ActionRepository, "update"> &
  Partial<Pick<ActionRepository, "get">>;
export type MarkActionNotDoneRepository = CompleteActionRepository;
type ActionTerminalClosurePatch = Omit<ActionRecordPatch, "state">;

export function completeAction(input: {
  actionId: ActionId;
  repository: CompleteActionRepository;
  patch?: ActionTerminalClosurePatch;
...
export function markActionNotDone(input: {
  actionId: ActionId;
  repository: MarkActionNotDoneRepository;
  patch?: ActionTerminalClosurePatch;
  tracer?: LifecycleTracer;
  turnId?: string;
  traceSource?: string;
