...
export const sharedStateMigrations = [
  {
    ...
    up: (db) => {
      db.exec(`
        ...
        CREATE INDEX idx_decision_artifact_entries_audience_rank
          ON decision_artifact_entries(audience_entity_id, rank ASC, created_at ASC);
        CREATE INDEX idx_decision_artifact_entries_audience_state_key
          ON decision_artifact_entries(audience_entity_id, state_key);
        CREATE INDEX idx_decision_artifact_entries_kind
          ON decision_artifact_entries(kind);
        CREATE INDEX idx_decision_artifact_entries_superseded
