    ...
    if (this.options.entryIndex !== undefined) {
      ...
      for (const entryId of [...pendingIds]) {
        ...
        if (!existingSessionIds.has(record.session_id)) {
          console.warn("Citation index referenced a missing session; falling back to stream scan.", {
            entryId,
        ...
        if (entry.id !== entryId) {
          console.warn("Citation index returned a mismatched stream entry; falling back to stream scan.", {
            entryId,
