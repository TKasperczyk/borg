...
const PROMPT_INJECTION_PNG_1X1 = Uint8Array.from([
  ...
  0x02, 0x00, 0x01, 0xe2, 0x21, 0xbc, 0x33, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e, 0x44,
  0xae, 0x42, 0x60, 0x82,
]);
...
function imageRefCount(messages: readonly LLMContentBlockMessage[]): number {
  return messages.reduce(
    (count, message) => count + message.content.filter((block) => block.type === "image_ref").length,
    0,
  );
}
