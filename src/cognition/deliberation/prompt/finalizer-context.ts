...
function renderCommitments(context: DeliberationContext): RenderedTerminalSection {
  ...
  return terminalSection(
    ...
    [
      ...
      "  <field_legend>Absolute record fields and the semantic field-set union of canonical scope/detail and standing-ledger rows are carried by each durable row plus its ID-keyed turn overlay. directive_exact, directive_excerpt_shape, directive_included_chars, and directive_total_chars state whether the directive is complete and, when cut, exactly how much source text is present. advisory_excerpt_budget_chars is the whole width a cut advisory directive is rendered into, not the share of that width the directive receives: the head+tail marker printed inside the excerpt is drawn from the same budget, so on a cut row directive_included_chars plus that marker's own length accounts for the budget and directive_included_chars can never reach it. The ceiling that leaves is a property of this render rather than of the directive: it moves between rows with the digit widths of the three numbers the marker carries, and falls a further character or two when a cut would land mid-character, so the two sum to at most the budget rather than always exactly to it. directive_exact reports elision only, not byte-fidelity of the printed attribute: values here are XML-attribute-encoded, so quotes, ampersands, angle brackets, newlines and tabs print as entities and the printed attribute runs longer than the counts beside it, which measure the stored string before encoding. That encoder emits no backslash, so a backslash inside a directive is stored content rather than an artifact of this render. On a critical row directive_exact is true by construction rather than by measurement, since critical directives are never cut; directive_excerpt_shape is full and directive_included_chars equals directive_total_chars for the same reason, so no field on such a row measures elision.</field_legend>",
      ...rows.map((row) => `  ${row}`),
