...
// the representative bounded high-water surface just under this ceiling.
...
function renderTurnStateMechanismHistory(
  ...
  return {
    lines: [
      context.turnMechanismEvidence?.answeredWindow === undefined
        ? '  <answered_window state="unavailable" />'
        : `  <answered_window disclosure="${disclosureAttribute}" interpretation="exact source-ID membership; scope labels never gate recall or response; before_window may include previously answered inputs">${escapeXmlText(JSON.stringify(context.turnMechanismEvidence.answeredWindow))}</answered_window>`,
      ...suppressionRows.map((row) => `  ${row}`),
      ...regenerationRows.map((row) => `  ${row}`),
    ],
    rowCount: 1 + suppressionRows.length + regenerationRows.length,
    truncationCount,
    omissionCount:
      Math.max(
        0,
