    ...
    return {
      text: [
        ...
        ...(goals.some((goal) => goal.status === "blocked" || (goal.block_history?.length ?? 0) > 0)
          ? [
              "  <block_state_legend>block_state carries the named blocker, attempted_unavailable declaration, blocked_at, reason, optional artifact handle, and prior block intervals with unblock bases. Absent means no block history recorded. Blocked goals remain recalled, unscored, with no progress debt or deadline pressure; closed intervals pause sd/sdebt clocks without changing actual progress or target dates.</block_state_legend>",
            ]
          : []),
