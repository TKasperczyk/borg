...
export function renderGoalDigest(context: DeliberationContext): RenderedPlannerSection {
  ...
  return section(
    "goal_index",
    [
      `<borg_planner_goal_digest rows_total="${goals.length}" target_tokens="${PLANNER_GOAL_TARGET_TOKENS}" focus_threshold="${context.executiveFocus === undefined || context.executiveFocus === null ? "none" : context.executiveFocus.threshold.toFixed(4)}" progress_debt_stale_ms="${context.executiveFocus === undefined || context.executiveFocus === null ? "none" : context.executiveFocus.score_basis.progress_debt_stale_ms}">`,
...
function renderCommitmentDigestText(input: {
  ...
  return [
    ...
    "  <field_legend>c row: i=id, s=status, ec=enforcement_class, cd=critical_domain, k=kind, t=type, cp=closure_pressure_relevance, p=priority, cat=created_at, ca=created_age, ra=reinforced_age, xa=expires_rel, dc=disclosure_class, oa=origin_audience, pt=private_to, pub=public_to, to=made_to, aud=restricted_audience, about=about_entity, by=committed_by_entity_id, ex=directive_exact, shape=directive_excerpt_shape, r=directive_rendered_chars, n=directive_total_chars, e=directive_elided_chars, f=directive_family, d=directive. ex reports elision only, not byte-fidelity of the printed attribute: d and f are XML-attribute-encoded, so quotes, ampersands, angle brackets, newlines and tabs print as entities while r/n/e count the stored string before encoding; that encoder emits no backslash, so a backslash inside d is stored content rather than an artifact of this render; and on a critical row ex is true by construction rather than by measurement, where shape and e are omitted entirely and r and n are both the stored length, so no attribute on such a row measures elision.</field_legend>",
