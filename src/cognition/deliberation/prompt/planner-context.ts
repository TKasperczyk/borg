...
export function renderGoalDigest(context: DeliberationContext): RenderedPlannerSection {
  ...
  return section(
    ...
    [
      ...
      "  <interpretation>The one-line index is complete for the globally assembled self snapshot, and that snapshot is assembled by goal status rather than from every goal on record: statuses_present on complete_goal_index lists the statuses this draw actually carries. A goal whose status is not listed there is absent from this page rather than omitted from it, so no count here is evidence about it in either direction. Status and ages are comparable across rows. Expanded rows are the highest global executive-score candidates, not an audience visibility filter.</interpretation>",
