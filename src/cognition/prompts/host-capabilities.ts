...
function wiredOutboundCapabilityLines(sourceTypes: readonly SessionSourceType[]): string[] {
  ...
  return [
    ...
    "- A post the connector accepted appends no event of its own, so delivery events in my stream can only ever be failures, and their absence is not evidence that nothing was delivered. Downstream of the delivery call the outcome is read in exactly one place, and only to withhold: the cross-session activity record for a post is written only when the connector accepted it, so a refused post leaves none.",
