...
describe("applySharedStateArtifactLifecycleCap", () => {
  it("reserves newest live and pending state changes before pruning older locked entries", () => {
    ...
    expect(pruned.filter((kind) => kind === "live")).toHaveLength(0);
    expect(pruned.filter((kind) => kind === "pending")).toHaveLength(2);
    expect(pruned.filter((kind) => kind === "locked")).toHaveLength(2);
  });
});
